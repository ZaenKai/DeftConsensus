...
export function BriefPane({
  ...
  if (uiState === "loading") {
    return <section className="surface-card p-4 text-sm text-muted">Loading brief…</section>;
  }
  if (uiState === "error") {
    return (
      <section className="surface-card border-danger/40 bg-danger/10 p-4 text-sm text-danger">
        Brief content is unavailable.
      </section>
    );
  }
  if (uiState === "empty") {
    return <section className="surface-card p-4 text-sm text-muted">No brief content has been drafted yet.</section>;
  }
  ...
  return (
    <section aria-label="Brief pane" className="h-full">
      <article className="surface-card h-full p-3 md:p-4" aria-label="Brief document">
        <div className="mx-auto w-full max-w-3xl rounded-md border border-[color:var(--color-surface-edge)] bg-[color:var(--color-surface)] px-5 py-5 md:px-8 md:py-8">
          <header className="flex flex-wrap items-start justify-between gap-2 border-b border-[color:var(--color-surface-edge)] pb-4">
            <div>
              <p className="text-xs uppercase tracking-[0.14em] text-muted">
                {activeView === "original" ? "Original snapshot · immutable" : "In progress · governed edits"}
              </p>
              <h2 className="mt-2 font-heading text-2xl font-bold text-text">Feature Brief</h2>
            </div>
            <div
              className="inline-flex items-center gap-1 rounded-full border border-[color:var(--color-surface-edge)] bg-[color:var(--color-bg)] p-1"
              role="tablist"
            ...
            >
              <button
                ...
                className={`rounded-full px-2 py-1 text-[10px] font-semibold ${
                  activeView === "original" ? "bg-[color:var(--color-surface)] text-text" : "text-muted"
                }`}
              ...
              <button
                type="button"
                role="tab"
                aria-selected={activeView === "in-progress"}
                className={`rounded-full px-2 py-1 text-[10px] font-semibold ${
                  activeView === "in-progress" ? "bg-[color:var(--color-surface)] text-text" : "text-muted"
                }`}
            ...
            {originalSections.map((section) => {
              ...
              return (
                <section key={section.id}>
                  ...
                  {activeView === "original" ? (
                    <p className="mt-3 whitespace-pre-wrap text-base leading-8 text-text">{section.content}</p>
                  ) : sectionUpdates.length === 0 ? (
                    <p className="mt-3 whitespace-pre-wrap text-base leading-8 text-text">{inProgressSection.content}</p>
                  ) : (
                    <div className="mt-3 space-y-3">
                      {review.entries.map((entry) => {
                        ...
                        const addedTextClass =
                          entry.state === "rejected"
                            ? "border-[color:var(--color-surface-edge)] bg-[color:var(--color-bg)] text-muted line-through"
                            : "border-success/30 bg-success/10 text-text";
                        const removedTextClass =
                          entry.state === "approved" || entry.state === "auto-applied"
                            ? "text-danger/75 line-through decoration-danger/70"
                            : "text-muted line-through";

                        return (
                          <article
                            key={entry.updateId}
                            className="group rounded-md border border-[color:var(--color-surface-edge)] bg-[color:var(--color-bg)]/35 p-3"
                          >
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <span className="dashboard-chip">{statusLabel(entry.state)}</span>
                            ...
                            <div className="mt-2 space-y-2">
                              ...
                              <div className={`whitespace-pre-wrap rounded-sm border border-l-2 px-2 py-1 text-sm leading-7 ${addedTextClass}`}>
                                {entry.newText}
                              </div>
                            </div>
                            {pending ? (
                              <div className="mt-3 flex justify-end">
                                <div className="flex items-center gap-2 md:opacity-0 md:pointer-events-none md:transition-opacity md:duration-150 md:group-hover:opacity-100 md:group-hover:pointer-events-auto md:group-focus-within:opacity-100 md:group-focus-within:pointer-events-auto">
                                  <button
                                    type="button"
                                    className="dashboard-nav-link px-2 py-1 text-xs"
                                  ...
                                  <button
                                    type="button"
                                    className="gradient-button px-2 py-1 text-xs font-semibold"
                      ...
                      <div className="rounded-md border border-[color:var(--color-surface-edge)] bg-[color:var(--color-bg)] p-3">
                        <p className="text-xs uppercase tracking-[0.1em] text-muted">Current in-progress text</p>
                        <p className="mt-2 whitespace-pre-wrap text-base leading-8 text-text">{review.currentText}</p>
                      </div>
                    </div>
                  )}
                </section>
